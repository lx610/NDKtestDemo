...
//    }
    ...
    public RequestIntercept(RequestHandler requestHandler) {
...
//            request = mHandler.onHttpRequestBefore(chain, request);
                if (requestHandler != null)//在请求服务器之前可以拿到request,做一些操作比如给request添加header,如果不做操作则返回参数中的request
            request = requestHandler.onHttpRequestBefore(chain, request);
//
...
//            return mHandler.onHttpResultResponse(bodyString, chain, originalResponse);
        if (requestHandler != null)//这里可以比客户端提前一步拿到服务器返回的结果,可以做一些操作,比如token超时,重新获取
            return requestHandler.onHttpResultResponse(bodyString, chain, originalResponse);
    ...
    public interface RequestHandler{
        Request onHttpRequestBefore(Chain chain, Request request);
        Response onHttpResultResponse(String bodyString, Chain chain, Response originalResponse);
    }
}
